...
$(function() {
	...
	$search
		.autocomplete(ApiGen.elements, {
			...
			var location = window.location.href.split('../api.html');
			location.pop();
			...
			window.location = location.join('../api.html');

			// Workaround for Opera bug
			$(this).closest('form').attr('action', location.join('../api.html'));
		}).closest('form')
			.submit(function() {
